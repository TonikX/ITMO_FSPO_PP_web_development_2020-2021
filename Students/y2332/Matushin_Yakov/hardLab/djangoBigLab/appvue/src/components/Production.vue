<template>
  <v-card class="mx-auto">
    <v-row>
      <v-col v-for="(item, i) in production" :key="i" cols="10" style="margin: 2%">
        <v-card :color="white" light>
          <div class="d-flex flex-no-wrap justify-space-between">
            <div>
              ...
              <v-card-actions>
               <router-link :to="{name: 'editItem', params: { id: item.id }}" class="btn btn-sm btn-primary">Edit</router-link>
              </v-card-actions>
              <v-card-actions>
  ...
  methods: {
    remove: function (e) {
      if (confirm('Delete ' + e.name)) {
        axios.delete(`http://127.0.0.1:8000/api/items/${e.id}`, {
          headers: {
