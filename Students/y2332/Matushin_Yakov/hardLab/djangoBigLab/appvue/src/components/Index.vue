<template>
  <v-card class="mx-auto">
    <v-row>
      <v-col v-for="(item, i) in employers" :key="i" cols="10" style="margin: 2%">
                ...
                <v-expansion-panels v-model="panel" :disabled="disabled">
                  <v-expansion-panel>
                    ...
                    <v-expansion-panel-content>
                      <b>Employer Code:</b> {{ item.code}}
                      <br />
                      <b>Employer Pass:</b> {{ item.pass_data }}
                      <br />
                    </v-expansion-panel-content>
                  </v-expansion-panel>
                </v-expansion-panels>
  ...
  methods: {
    remove: function (e) {
      if (confirm('Delete ' + e.name)) {
        axios.delete(`http://127.0.0.1:8000/api/employers/${e.id}`, {
          headers: {
