  ...
  <v-row align="center" class="list px-3 mx-auto">
    ...
    <v-col cols="12" sm="12">
      <v-card class="mx-auto" tile>
        <v-card-title>Legal Owners</v-card-title>
...
export default {
  ...
  data() {
    return {
      ...
      search: "",
      headers: [
        { text: "INN", align: "start", value: "owner_inn", sortable: false },
        { text: "Name", value: "owner_name", sortable: false },
        { text: "Chief", value: "chief", sortable: false },
  ...
  methods: {
    retrieveItems() {
      DataService.getAll()
        .then((response) => {
          this.items = response.data.data.map(this.getDisplayLOwner);
    ...
    addItem() {
      this.$router.push({ name: "add-lOwner" });
    },
    ...
    getDisplayLOwner(lOwner) {
      let attributes = lOwner.attributes
      return {
        id: lOwner.id,
        owner_inn: attributes.owner_inn,
