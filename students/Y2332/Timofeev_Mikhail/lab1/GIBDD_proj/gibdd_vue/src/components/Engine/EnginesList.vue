  ...
  <v-row align="center" class="list px-3 mx-auto">
    ...
    <v-col cols="12" sm="12">
      <v-card class="mx-auto" tile>
        <v-card-title>Engines</v-card-title>
...
export default {
  ...
  data() {
    return {
      ...
      search: "",
      headers: [
        { text: "Type", align: "start", value: "engine_type", sortable: false },
  ...
  methods: {
    retrieveItems() {
      DataService.getAll()
        .then((response) => {
          this.items = response.data.data.map(this.getDisplayEngine);
    ...
    addItem() {
      this.$router.push({ name: "add-engine" });
    },
    ...
    getDisplayEngine(engine) {
      let attributes = engine.attributes
      return {
        id: engine.id,
        engine_type: attributes.engine_type,
